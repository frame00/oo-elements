...
describe(`<${ELEMENT}></${ELEMENT}>`, () => {
	...
	describe('Observe Document events', () => {
		it('Add <oo-notification> when "oonotification" event fires', done => {
			...
			document.addEventListener('oonotification', (e: CustomEvent) => {
				const notification = element.shadowRoot.querySelector('oo-notification').querySelector('[slot=body]')
				expect(notification.textContent).to.be('test')
				done()
			}, {once: true})
			document.dispatchEvent(new CustomEvent('oonotification', {detail: {
				message: 'test'
			}}))
		})
	})
	...
	describe('Remove notification', () => {
		it('Delete <oo-notification> when <oo-notification> clicked', done => {
			const element = insertElement(ELEMENT)
			document.addEventListener('oonotification', async (e: CustomEvent) => {
				const notification = element.shadowRoot.querySelector('oo-notification')
				expect(notification).to.be.ok()

				event(notification, 'click')
				await sleep(100)

				const notificationNext = element.shadowRoot.querySelector('oo-notification')
