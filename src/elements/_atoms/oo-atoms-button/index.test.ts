...
describe(`<${ELEMENT}></${ELEMENT}>`, () => {
	...
	describe('Apply status with "data-status" attribute', () => {
		it('progress', () => {
			const element = insertElement(ELEMENT, new Map([['data-state', 'progress']]))
			expect(element.shadowRoot.querySelector('button').classList.contains('progress')).to.be.ok()
		})

		it('resolved', () => {
			const element = insertElement(ELEMENT, new Map([['data-state', 'resolved']]))
			expect(element.shadowRoot.querySelector('button').classList.contains('resolved')).to.be.ok()
		})

		it('rejected', () => {
			const element = insertElement(ELEMENT, new Map([['data-state', 'rejected']]))
			expect(element.shadowRoot.querySelector('button').classList.contains('rejected')).to.be.ok()
		})

		it('Other as empty', () => {
			const element = insertElement(ELEMENT, new Map([['data-state', 'xxx']]))
			expect(element.shadowRoot.querySelector('button').classList.contains('xxx')).to.be(false)
		})
	})
