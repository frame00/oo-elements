...
export default class extends OOElement {
	...
	render() {
		const {
			...
			stripeUser,
			notificationEMail,
			notificationEMailServiceInformation,
			buttonState
		} = {
			...
			notificationEMailServiceInformation: stateNotificationEMailServiceInformation.get(
				this
			),
		...
		const options = [
			{
				name: 'uid',
				title: 'Your ID',
				template: html`<input name=iam type=text value$='${iam}' disabled></input>`
			},
			{
				name: 'permalink',
				title: 'User name',
				template: html`<oo-user-name-editor data-iam$='${iam}'></oo-user-name-editor>`
			},
			{
				name: 'name',
				title: 'Display name',
				template: html`<input name=name type=text value$='${name}' on-change='${e =>
					this.onChange(e, 'name')}' required></input>`
			},
			{
				name: 'bio',
				title: 'Profile',
				template: html`<textarea name=bio on-change='${e =>
					this.onChange(e, 'bio')}'>${bio}</textarea>`
			},
			{
				name: 'stripe',
				title: 'Stripe',
			...
			},
			{
				...
				template: html`
				<label>
					<input name=notifications_opt_email type=checkbox checked?='${notificationEMail}' on-change='${e =>
					this.onChange(e, 'notifications_opt_email')}'></input>
					New project and message
				</label>
				<label>
					<input name=notifications_opt_email_service_information type=checkbox checked?='${notificationEMailServiceInformation}' on-change='${e =>
					this.onChange(
						e,
						'notifications_opt_email_service_information'
					)}'></input>
			...
			},
			{
				...
				template: html`<input name=email type=email value$='${email}' on-change='${e =>
					this.onChange(e, 'email')}' required></input>`
			}
		]
		return html`
		<style>
	...
	async fetchUserData(uid: string) {
		...
		if (Array.isArray(response)) {
			...
			stateNotificationEMailServiceInformation.set(
				this,
				ext.get('notifications_opt_email_service_information')
			)
			this.update()
		} else {
			this.update()
	...
	onChange(
		e: HTMLElementEvent<HTMLInputElement | HTMLTextAreaElement>,
		name: string
	) {
		const { target } = e
		const { value } = target
	...
	async onSubmit(e: Event) {
		...
		const extensions = {
			...
			notifications_opt_email_service_information: stateNotificationEMailServiceInformation.get(
				this
			)
		}
