...
export default (
	...
	return html`
	<p>USD</p>
	<input name=usd type=number step=0.01 min=5 value$='${usd}' on-change='${e =>
		callbacks.usd(e, 'usd')}'></input>
	<p>JPY</p>
	<input name=jpy type=number step=1 min=500 value$='${jpy}' on-change='${e =>
		callbacks.jpy(e, 'jpy')}'></input>
	`
}
